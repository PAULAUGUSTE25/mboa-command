...
// PostgreSQL-compatible query helpers (sync wrappers)
function queryAll(sql, params = []) {
  const converted = convertPostgresToSQLite(sql);
  // Replace ILIKE with LIKE for SQLite (case-insensitive by default in SQLite with LIKE)
  const sqliteSql = converted.replace(/ILIKE/g, 'LIKE');
...
function run(sql, params = []) {
  const converted = convertPostgresToSQLite(sql);
  return db.prepare(converted).run(...params);
}
...
// Export PostgreSQL-compatible API
module.exports = {
  pool: undefined,
  queryAll,
  queryOne,
